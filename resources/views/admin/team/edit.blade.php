...
@if (is_permited('browse_admins') == 1)
    ...
    <div class="col-md-12 col-xl-12">
        <div class="card">
            ...
            <form action="{{ aurl('team/update/' . $employee->id) }}" enctype="multipart/form-data" method="POST">
                @csrf
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label class="floating-label" for="position_ar">{{ trans('admin.Position Ar') }} <span
                                class="redStar">*</span></label>
                        <input type="text" value="{{ $employee->position_ar }}" name="position_ar" class="form-control"
                            id="position_ar">
                    </div>

                    <div class="form-group">
                        <label class="floating-label" for="position_en">{{ trans('admin.Position En') }} <span
                                class="redStar">*</span></label>
                        <input type="text" value="{{ $employee->position_en }}" name="position_en" class="form-control"
                            id="position_en">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" class="form-select" id="status">
                            <option value="1" {{($employee->status == 1)?'selected':''}}>{{trans('admin.Active')}}</option>
                            <option value="0" {{($employee->status == 0)?'selected':''}}>{{trans('admin.DeActive')}}</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <button type="button" class="btn btn-pill btn-outline-primary btn-air-primary"
                            ...
                            <img src="{{ ($employee->avatar()->exists())?$employee->avatar->url : asset('dashboard/assets/images/placeholder-image.png') }}"
