            ...
            <div class="card-block row">
                <div class="col-sm-12 col-lg-12 col-xl-12">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                @foreach ($team as $employee)
                                    <tr>
                                        <td>{{ $employee->id }}</td>
                                        <td><img src="{{ ($employee->avatar()->exists())?$employee->avatar->url:"" }}" style="height:80px" alt=""></td>
                                        <td>{{ $employee->name }}</td>
                                        ...
                                        <td>
                                            @if (is_permited('edit_team') == 1)
                                            <a href="{{ aurl('team/edit/' . $employee->id) }}"
                                                class="btn btn-pill btn-outline-warning btn-air-warning"><i
                                                    class="fas fa-edit"></i>
                                                {{ trans('admin.Edit') }}</a>
                                                @endif

                                                @if (is_permited('delete_team') == 1)
                                            <button data-id="{{ $employee->id }}" data-name="{{ $employee->name }}"
                                                data-url="{{ aurl('team/'.$employee->id.'/delete') }}"
                        ...
                        {{ $team->links('admin.pagination.index') }}
                    </div>
                </div>
            </div>
        </div>
