...
@if (is_permited('browse_admins') == 1)
    ...
    <div class="col-md-12 col-xl-12">
        <div class="card">
            ...
            <form action="{{ aurl('options/update/' . $option->id) }}" enctype="multipart/form-data" method="POST">
                @csrf
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" value="{{ $option->name_ar }}" name="name_ar" class="form-control"
                            id="name_ar">
                    </div>
                    ...
                    <div class="form-group">
                        <label class="floating-label" for="name_en">{{ trans('admin.Name En') }} <span
                                class="redStar">*</span></label>
                        <input type="text" value="{{ $option->name_en }}" name="name_en" class="form-control"
                            id="name_en">
                    </div>
                    <div class="form-group">
                        <select name="status" class="form-select" id="status">
                            <option value="1" {{($option->status == 1)?'selected':''}}>{{trans('admin.Active')}}</option>
                            <option value="0" {{($option->status == 0)?'selected':''}}>{{trans('admin.DeActive')}}</option>
                        </select>
                    </div>
