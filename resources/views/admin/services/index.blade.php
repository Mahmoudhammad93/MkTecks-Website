            ...
            <div class="card-block row">
                <div class="col-sm-12 col-lg-12 col-xl-12">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                @foreach ($services as $service)
                                    <tr>
                                        <td>{{ $service->id }}</td>
                                        <td><img src="{{ ($service->image()->exists())?$service->image->url:"" }}" style="height:80px" alt=""></td>
                                        <td>{{ $service->title }}</td>
                                        ...
                                        <td>
                                            @if (is_permited('edit_services') == 1)
                                            <a href="{{ aurl('services/edit/' . $service->id) }}"
                                                class="btn btn-pill btn-outline-warning btn-air-warning"><i
                                                    class="fas fa-edit"></i>
                                                {{ trans('admin.Edit') }}</a>
                                                @endif

                                                @if (is_permited('delete_services') == 1)
                                            <button data-id="{{ $service->id }}" data-name="{{ $service->name }}"
                                                data-url="{{ aurl('services/'.$service->id.'/delete') }}"
                        ...
                        {{ $services->links('admin.pagination.index') }}
                    </div>
                </div>
            </div>
        </div>
